    ...
    )
{
  ...
  // TODO P1 if ( ( pp[1] & 0xFF ) != XT_ARRAY_DOUBLE ) { go_BYE(-1); }
  if ( ( pp[1] & 0xFF ) != XT_ARRAY_DOUBLE ) { status = -1; goto BYE; }
  uint32_t len = pp[1] >> 8;
  num_left_to_read -= 8;
  if ( num_left_to_read != (int)len ) { go_BYE(-1); }
  int n = num_left_to_read / sizeof(double);
  if ( n != 1 ) { go_BYE(-1); }
  nr = read(sock, ptr_dval, sizeof(double));
  if ( nr == 0 ) { go_BYE(-2); } 
  if ( nr <  0 ) { go_BYE(-3); } 
BYE:
  free_if_non_null(param1);
